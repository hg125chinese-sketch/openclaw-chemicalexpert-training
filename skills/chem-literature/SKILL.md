...
- **When to use:** when you want a first-class SDK path for literature search instead of hand-written PubMed/arXiv/bioRxiv requests.
- **Tool names:** `PubMed_search_articles`, arXiv / bioRxiv search tools exposed by your ToolUniverse install.
- **Code example:**
...
papers = tu.run({
    ...
    "arguments": {"query": "TGFBR1 idiopathic pulmonary fibrosis", "max_results": 10}
})
```
